...
const MarloweDatumInfo: React.FC<Props> = ({ bytes, scriptHash }) => {
  ...
  try {
    ...
    const decode: any = (window as any).marlowe_rs?.decode_cborhex_marlowe_plutus_datum;
    if (decode === undefined) {
      return <></>;
    }
    ...
    return (
      <Container>
        <Typography variant="subtitle1">Decoded Marlowe Datum:</Typography>
        <br />
        <pre>{decoded}</pre>
      </Container>
    );
...
export default MarloweDatumInfo;
